...
class PlgSystemJSNTPLFrameworkInstallerScript
{
	 ...
	 * @var boolean
	 */
	private $_downloadFramework = true;
	...
	public function preflight ($mode, $parent)
	{
		...
		try
		{
			if ($this->_checkPermission())
			{
				...
				if ($this->_isInstalled('jsntplframework'))
				{
	...
	public function postflight ($mode, $parent)
	{
		...
		try
		{
			...
			self::$_installer		= $parent->getParent();
			self::$_manifest		= self::$_installer->getManifest();
			self::$_installPath		= dirname(__FILE__);
			self::$_templateName	= (string) self::$_manifest->template;

			// Download framework package and extract it
			// to plugin folder
			...
			// Execute query
			method_exists(self::$_dbo, 'execute')
				? self::$_dbo->execute()
				: self::$_dbo->query();
		}
		catch (Exception $ex)
	...
	private function _migrateParams ($templateName)
	{
		...
		foreach (self::$_dbo->loadObjectList() AS $row)
		{
			...
			$query->set("params = '" . str_replace('\\', '\\\\', json_encode($newParams)) . "'");
	...
	private function _installFramework ()
	{
		if (is_file(self::$_installPath . '/backups/jsntplframework.xml'))
		{
			...
			// Get latest version of template framework
			...
			if (version_compare($frameworkVersion, $latest, '>='))
			{
		...
		{
			throw new Exception('PLG_SYSTEM_JSNTPLFRAMEWORK_ERROR_CANNOT_DOWNLOAD_FRAMEWORK_PACKAGE');
		}
		...
		self::$_app->enqueueMessage('Installed framework from joomlashine server');
	}
